...
/// Treat a [`sync::BulkProducer`](crate::sync::BulkProducer) as a [`std::io::BufRead`].
#[derive(Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Debug)]
pub struct BulkProducerToBufRead<P>(P);

impl<P> BulkProducerToBufRead<P> {
    /// Wrap a [`sync::BulkProducer`](crate::sync::BulkProducer) as a [`std::io::BufRead`].
    pub fn new(producer: P) -> Self {
        Self(producer)
    }
}

impl<P> From<P> for BulkProducerToBufRead<P> {
    fn from(value: P) -> Self {
        Self(value)
    }
}

impl<P> Wrapper<P> for BulkProducerToBufRead<P> {
    fn into_inner(self) -> P {
        self.0
    }
}

impl<P> AsRef<P> for BulkProducerToBufRead<P> {
    fn as_ref(&self) -> &P {
        &self.0
    }
}

impl<P> AsMut<P> for BulkProducerToBufRead<P> {
...
impl<P> Read for BulkProducerToBufRead<P>
where
    P: BulkProducer<Item = u8, Final = (), Error = io::Error>,
{
    /// Translates the final value of the producer into a return of `Ok(0)`.
    /// Hence, `read` must not be called after it returns `Ok(0)` (or any error, for that matter).
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        match self.0.bulk_produce(buf)? {
            Left(amount) => Ok(amount),
            Right(()) => Ok(0),
        }
    }
}
...
/// not return a result.
impl<P> BufRead for BulkProducerToBufRead<P>
where
    P: BulkProducer<Item = u8, Final = (), Error = io::Error>,
{
    /// Translates the final value of the producer into a return of the empty slice.
...
/// Treat a [`BufRead`] as an [`sync::BulkProducer`](crate::sync::BulkProducer).
#[derive(Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Debug)]
pub struct BufReadToBulkProducer<R>(R);

impl<R> BufReadToBulkProducer<R> {
    /// Wrap a [`BufRead`] as an [`sync::BulkProducer`](crate::sync::BulkProducer).
    pub fn new(reader: R) -> Self {
        Self(reader)
    }
}

impl<R> From<R> for BufReadToBulkProducer<R> {
    fn from(value: R) -> Self {
        Self(value)
    }
}

impl<R> Wrapper<R> for BufReadToBulkProducer<R> {
    fn into_inner(self) -> R {
        self.0
    }
}

impl<R> AsRef<R> for BufReadToBulkProducer<R> {
    fn as_ref(&self) -> &R {
        &self.0
    }
}

impl<R> AsMut<R> for BufReadToBulkProducer<R> {
    fn as_mut(&mut self) -> &mut R {
        &mut self.0
    }
}

impl<R> Producer for BufReadToBulkProducer<R>
where
    R: BufRead,
{
    type Item = u8;
...
impl<R> BufferedProducer for BufReadToBulkProducer<R>
where
    R: BufRead,
{
...
impl<R> BulkProducer for BufReadToBulkProducer<R>
where
    R: BufRead,
{
...
impl<R, const N: usize> From<(R, Static<u8, N>)> for ReadToBulkProducer<R, N> {
    fn from((reader, buf): (R, Static<u8, N>)) -> Self {
        Self { reader, buf }
    }
}

impl<R, const N: usize> Wrapper<(R, Static<u8, N>)> for ReadToBulkProducer<R, N> {
...
impl<R, const N: usize> Producer for ReadToBulkProducer<R, N>
where
    R: Read,
{
    type Item = u8;
...
impl<R, const N: usize> BufferedProducer for ReadToBulkProducer<R, N>
where
    R: Read,
{
...
impl<R, const N: usize> BulkProducer for ReadToBulkProducer<R, N>
where
    R: Read,
{
    /// Signals `Final` if the inner `read` method ever produces no bytes.
