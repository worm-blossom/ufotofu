...
pub struct BulkProducerToAsyncBufRead<P>(P);
    ...
    P: BulkProducer<Item = u8, Final = (), Error = io::Error>,
{
    ...
    /// Hence, `read` must not be called after it returns `Ok(0)` (or any error, for that matter).
    fn read(&mut self, buf: &mut [u8]) -> futures::Result<usize> {
...
/// Treat a [`AsyncBufRead`] as an [`local_nb::BulkProducer`](crate::local_nb::BulkProducer).
#[derive(Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Debug)]
pub struct AsyncBufReadToBulkProducer<R>(R);

impl<R> AsyncBufReadToBulkProducer<R> {
    /// Wrap a [`AsyncBufRead`] as an [`local_nb::BulkProducer`](crate::local_nb::BulkProducer).
