...
/// A sequence of heap-allocated operations for determining the method call
/// patterns that a `Scramble` wrapper subjects its inner consumer to. Intended
/// to be [generated by a fuzzer](https://rust-fuzz.github.io/book/cargo-fuzz/structure-aware-fuzzing.html).
...
/// A `Consumer` wrapper that scrambles the methods that get called on a wrapped
/// consumer, without changing the observable semantics. Unless it uncovers
/// buggy behavior on the wrapped consumer, that is.
#[derive(Debug)]
...
impl<C, T, F, E> Scramble<C, T, F, E> {
    /// Create a new wrapper around `inner` that exercises the consumer trait
    /// methods of `inner` by cycling through the given `operations`. To provide
    /// this functionality, the wrapper must allocate an internal buffer of
    /// items, `capacity` sets the size of that buffer. Larger values allow for
    /// more bizarre method call patterns, smaller values consume less space
    /// (surprise!).
