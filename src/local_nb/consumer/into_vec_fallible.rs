...
impl<T> LocalConsumer for IntoVecFallibleInner<T> {
    type Item = T;
    type Final = ();
    type Error = IntoVecError;
...
impl<T> LocalBufferedConsumer for IntoVecFallibleInner<T> {
    async fn flush(&mut self) -> Result<Self::Final, Self::Error> {
        Ok(())
    }
}

impl<T: Copy> LocalBulkConsumer for IntoVecFallibleInner<T> {
    async fn consumer_slots<'a>(
        &'a mut self,
    ) -> Result<&'a mut [MaybeUninit<Self::Item>], Self::Error>
    where
