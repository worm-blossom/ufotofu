...
fuzz_target!(|data: (TestProducer<u16, u16, u16>, Box<[NonZeroUsize]>)| {
    ...
    while slice_length_index < slice_lengths.len() {
        ...
        match pro.bulk_produce(&mut produced[produced_so_far..]) {
            Ok(Left(num_consumed)) => {
                for _ in 0..num_consumed {
                    let control_item = control.produce().unwrap().unwrap_left();
                    produced_control.push(control_item);
                    produced_so_far += 1;
                }
            }
            Ok(Right(fin)) => {
                assert_eq!(fin, control.produce().unwrap().unwrap_right());
                break;
            }
            Err(err) => {
                assert_eq!(err, control.produce().unwrap_err());
                break;
            }
        }
