...
use ufotofu::{consumer::move_into_owned_slice, prelude::*, queues::new_fixed};
...
fuzz_target!(
    |data: (usize, Vec<BulkConsumerOperation>, Box<[u16]>, usize)| {
        pollster::block_on(async {
            ...
            let backing_vec = vec![17; slice_len];

            let mut con = move_into_owned_slice(backing_vec.clone())
            ...
            if slice_len >= input.len() {
                ...
                assert_eq!(&con.into_inner().into_inner()[..input.len()], &input[..]);
            } else if !did_error {
            ...
            let mut con2 = move_into_owned_slice(backing_vec.clone());
            ...
            assert_eq!(&con2.into_inner()[..min_len], &input[..min_len]);
        });
    }
);
