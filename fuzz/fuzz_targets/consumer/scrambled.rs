...
// Generate a TestConsumer, turn it into a scrambled, feed a sequence of values into it, and test that the wrapped TestConsumer then stores the same items as a non-scrambled control TestConsumer.
...
fuzz_target!(|data: (
    ...
    pollster::block_on(async {
        ...
        let mut scrambled = con.scrambled(new_fixed(buffer_size), ops);
